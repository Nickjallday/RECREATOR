  ...
  static void lcd_move_e(TERN_(MULTI_E_MANUAL, const int8_t eindex=active_extruder)) {
    ...
    if (ui.should_draw()) {
      ...
      MenuEditItemBase::draw_edit_screen(
        ...
        ui16tostr5rj(current_position.e
...
void _menu_move_distance(const AxisEnum axis, const screenFunc_t func, const int8_t eindex=active_extruder) {
  ...
  if (parser.using_inch_units()) {
    ...
    if (LARGE_AREA_TEST) SUBMENU(MSG_MOVE_1000MM, []{ _goto_manual_move(1000);    });
...
void menu_move() {
  ...
  // ------------------------------------------------------------------------------229 - 296 commented out for 3D Recreator
...
void menu_motion() {
  ...
  //
  // Auto Home ------------------------ 351-369 commented out for 3D Recreator
  //
  //GCODES_ITEM(MSG_AUTO_HOME, G28_STR);
